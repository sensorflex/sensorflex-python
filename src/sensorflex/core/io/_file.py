...
# Example leaf operators using nested Pydantic models only
...
class Y(Operator):
    class Input(BaseModel):
        x: int
        y: int  # NOTE: non-optional; X.Output.y is Optional[int] → compatible
...
def _cond_big(v: Add100.Input) -> bool:
    return v.z > 10.0
